...
//! - Target price moved by ≥ 1 tick on either side
//! - `min_requote_interval_ms` elapsed (forced refresh)

use tikr_core::{
    Decimal, MarketEvent, Price, QuoteKind, Side, Size, Snapshot, Symbol, TimeInForce, Timestamp,
};
...
impl TopOfBook {
    /// Compute the (bid, ask) target prices given a book snapshot.
    ...
    fn compute_targets(&self, snapshot: &Snapshot) -> Option<(Price, Price)> {
        ...
        if spread_ticks > threshold {
            ...
            Some((Price(best_bid.0 + tick), Price(best_ask.0 - tick)))
        } else {
