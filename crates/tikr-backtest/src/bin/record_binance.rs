...
/// Record a Binance market-data stream into parquet.
#[derive(Parser, Debug)]
#[command(
    name = "record_binance",
    about = "Record Binance market data (depth + aggTrade) to parquet"
)]
struct Args {
    /// Binance environment.
    ...
    /// Binance symbol (e.g. `BTCUSDT`). The base+quote are inferred via a
    /// simple split (USDT/USDC/BUSD/TUSD suffix).
...
async fn main() {
    ...
    let mut depth = match depth_stream::subscribe_depth(env, symbol.clone()).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("depth subscribe failed: {e}");
            std::process::exit(1);
        }
    };
    let mut trades = match trade_stream::subscribe_trades(env, symbol.clone()).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("trade subscribe failed: {e}");
            std::process::exit(1);
        }
    };
    ...
    loop {
        ...
        tokio::select! {
            ev = depth.next() => match ev {
                ...
                Some(_) => {}
                None => { warn!("depth stream ended"); break; }
            },
            ev = trades.next() => match ev {
                ...
                Some(_) => {}
                None => { warn!("trade stream ended"); break; }
            },
